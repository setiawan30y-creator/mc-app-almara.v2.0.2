...
class CustomerController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data = [
            ...
            'identity_number' => $input['no_ktp'] ?? '-',
            'selain_ktp' => $input['selain_ktp'] ?? '-',
        ...
        try {
            foreach ($customers as $index => $input) {
                ...
                $data = [
                    ...
                    'identity_number' => substr(trim($input['no_ktp'] ?? $input['identity_number'] ?? '-'), 0, 100),
                    'selain_ktp' => substr(trim($input['selain_ktp'] ?? '-'), 0, 100),
                    ...
                    'registration_date' => $registrationDate,
                    'customer_type' => substr(trim($input['kn'] ?? $input['customer_type'] ?? '1'), 0, 50),
                    'raw_json' => json_encode($input)
                ];
    ...
    public function destroy(Request $request)
    {
        ...
        try {
            ...
            Customer::where('internal_id', $id)->delete();
            return response()->json([
                'status' => 'success', 
                'message' => 'Data Nasabah Dihapus (Laravel)'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'status' => 'error', 
