...
window.toggleSelectAllResetModules = function(selectAll) {
    const checkboxes = document.querySelectorAll('.reset-module-checkbox');
...
window.executeSelectiveReset = async function() {
    const checkboxes = document.querySelectorAll('.reset-module-checkbox:checked');
    if (checkboxes.length === 0) {
        if (typeof Swal !== 'undefined') {
            Swal.fire('Pilih Modul', 'Silakan centang setidaknya satu modul/halaman yang ingin di-reset.', 'warning');
        } else {
            alert('Silakan centang setidaknya satu modul/halaman yang ingin di-reset.');
        }
        return;
    }
    ...
    const selectedLabels = Array.from(checkboxes).map(cb => {
        ...
        return parentLabel ? parentLabel.querySelector('strong')?.textContent || cb.value : cb.value;
    });

    let userInput = '';
    if (typeof Swal !== 'undefined') {
        const { value: text, isConfirmed } = await Swal.fire({
            title: 'Konfirmasi Reset Modul Terpilih',
            html: '<div style="text-align:left; font-size:0.9rem; margin-bottom:12px;">Modul yang akan di-reset:<ul style="margin-top:6px; margin-bottom:12px; padding-left:20px;">' +
                  selectedLabels.map(l => '<li style="color:#ef4444; font-weight:600;">' + l + '</li>').join('') +
                  '</ul><div style="font-size:0.82rem; color:#10b981; margin-bottom:10px;"><i class="fa-solid fa-lock"></i> Data RWT (Audit Trail) tetap aman dan terproteksi.</div>' +
                  'Ketik <b>RESET</b> untuk melanjutkan:</div>',
            input: 'text',
            inputPlaceholder: 'RESET',
            showCancelButton: true,
            confirmButtonColor: '#ef4444',
            confirmButtonText: 'Ya, Reset Modul Terpilih',
            cancelButtonText: 'Batal',
            background: '#1e293b',
            color: '#f8fafc',
            inputValidator: (value) => {
                if (value !== 'RESET') {
                    return 'Ketik "RESET" (huruf besar semua) untuk mengonfirmasi!';
                }
            }
        });
        if (!isConfirmed || text !== 'RESET') return;
        userInput = text;
    } else {
        const confirmMessage = 'PERINGATAN DANGER ZONE!\n\nAnda akan me-reset modul berikut:\n- ' + selectedLabels.join('\n- ') + '\n\nKetik "RESET" (huruf besar semua) untuk mengonfirmasi:';
        userInput = prompt(confirmMessage);
        if (userInput !== 'RESET') {
            alert('Kata kunci salah atau dibatalkan.');
            return;
        }
    }

    console.log('Menjalankan Reset Per Modul:', selectedValues);
    let resetSummary = [];

    // 1. Transactions
    if (selectedValues.includes('transactions')) {
        try {
            await fetch('api/transactions/clear-all', { method: 'DELETE' });
        } catch(e) {}
        localStorage.setItem('mc_transactions', JSON.stringify([]));
        localStorage.removeItem('mc_invoice_seq');
        resetSummary.push('Data Transaksi Operasional');
    }

    // 2. Customers
    if (selectedValues.includes('customers')) {
        try {
            await fetch('api/customers/clear-all', { method: 'POST' });
        } catch(e) {}
        localStorage.removeItem('mc_customers');
        localStorage.setItem('mc_customers_reset_at', new Date().toISOString());
        window.__mcCustomersMemory = [];
        resetSummary.push('Data Nasabah (KYC)');
    }

    // 3. Cash & Bank
    if (selectedValues.includes('cash_bank')) {
        localStorage.setItem('mc_cash', '0');
        localStorage.setItem('mc_bank_bca', '0');
        localStorage.setItem('mc_bank_mandiri', '0');
        resetSummary.push('Saldo Kas & Bank');
    }

    // 4. Currency Stock
    if (selectedValues.includes('currency_stock')) {
        if (typeof getCurrencies === 'function' && typeof saveCurrencies === 'function') {
            let currencies = getCurrencies();
            currencies = currencies.map(c => { c.stock = 0; return c; });
            saveCurrencies(currencies);
        }
        resetSummary.push('Stok Valas');
    }

    // 5. Mutations & Adjustments
    if (selectedValues.includes('mutations')) {
        localStorage.setItem('mc_mutations', JSON.stringify([]));
        localStorage.setItem('mc_adjustments', JSON.stringify([]));
        resetSummary.push('Mutasi & Penyesuaian Saldo');
    }

    // 6. Closings & LKU
    if (selectedValues.includes('closings')) {
        localStorage.setItem('mc_closings', JSON.stringify([]));
        resetSummary.push('Riwayat Closing & LKU');
    }

    // 7. Expenses & Assets
    if (selectedValues.includes('expenses_assets')) {
        localStorage.setItem('mc_expenses', JSON.stringify([]));
        localStorage.setItem('mc_assets', JSON.stringify([]));
        resetSummary.push('Pengeluaran & Aset');
    }

    // 8. Old Money Transactions
    if (selectedValues.includes('old_money_trxs') || selectedValues.includes('old_money')) {
        localStorage.setItem('mc_old_money_trxs', JSON.stringify([]));
        resetSummary.push('Uang Lama/Koin (Riwayat Transaksi)');
    }

    // 9. Old Money Suppliers
    if (selectedValues.includes('old_money_suppliers') || selectedValues.includes('old_money')) {
        localStorage.setItem('mc_old_money_suppliers', JSON.stringify([]));
        resetSummary.push('Uang Lama/Koin (Supplier)');
    }

    // 10. Old Money Definitions / Items
    if (selectedValues.includes('old_money_items') || selectedValues.includes('old_money')) {
        localStorage.setItem('mc_old_money_items', JSON.stringify([]));
        resetSummary.push('Uang Lama/Koin (Definisi/Katalog)');
    }

    // 11. RWT (Audit Transaksi) - Khusus jika dicentang oleh user
    if (selectedValues.includes('rwt')) {
        try {
            await fetch('api/audit/clear-all', { method: 'DELETE' });
        } catch(e) {}
        localStorage.setItem('mc_transactions_audit', JSON.stringify([]));
        resetSummary.push('Data RWT (Audit Transaksi)');
    }

    // 12. HRIS & Employees
    if (selectedValues.includes('hris')) {
        localStorage.removeItem('mc_hris_employees');
        localStorage.removeItem('mc_hris_attendances');
        resetSummary.push('HRIS & Karyawan');
    }

    const successMsg = 'Modul berikut berhasil di-reset:\n- ' + resetSummary.join('\n- ') + '\n\nAplikasi akan memuat ulang.';
    if (typeof Swal !== 'undefined') {
        await Swal.fire({
            icon: 'success',
            title: 'Reset Modul Berhasil',
            text: 'Modul terpilih telah dibersihkan.',
            background: '#1e293b',
            color: '#f8fafc'
        });
    } else {
