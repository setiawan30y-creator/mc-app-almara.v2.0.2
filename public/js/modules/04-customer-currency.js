...
function filterCustomersTable() {
    ...
    customers.forEach(c => {
        ...
        // KN pada tabel KYC harus berupa kode data, bukan label tampilan.
        const rawKn = String(c.kn ?? c.customer_type ?? '').trim().toLowerCase();
        const knCode = rawKn === '2' || rawKn.includes('perusahaan') || rawKn.includes('corporate') ? '2' : '1';
        const savedIdentityType = String(c.jenis_id || c.identity_type || '').trim().toUpperCase();
        const validIdentityTypes = new Set(['KTP', 'SIM', 'PASSPORT', 'KITAS', 'SERTIFIKAT']);
        // Kolom Jenis ID selalu menunjukkan tipe yang dipilih pada form.
        // Untuk data lama tanpa jenis_id, lakukan inferensi seperlunya.
        const jenisId = validIdentityTypes.has(savedIdentityType)
            ? savedIdentityType
            : (c.no_ktp && c.no_ktp !== '-' ? 'KTP' : (c.selain_ktp && c.selain_ktp !== '-' ? 'LAINNYA' : '-'));
        const isKtpDocument = jenisId === 'KTP';
        const noKtp = isKtpDocument && c.no_ktp && c.no_ktp !== '-' ? c.no_ktp : '-';
        // Data lama mungkin masih menaruh dokumen non-KTP di no_ktp; tampilkan
        // tetap nomornya pada ID Lain agar dokumen tidak hilang.
        const otherDocumentNumber = !isKtpDocument
            ? (c.selain_ktp && c.selain_ktp !== '-' ? c.selain_ktp : (c.no_ktp && c.no_ktp !== '-' ? c.no_ktp : ''))
            : (c.selain_ktp && c.selain_ktp !== '-' ? c.selain_ktp : '');
        const idLain = otherDocumentNumber || '-';
