...
window.updateClosingModeUi = function() {
    ...
    if(help) {
        help.textContent = type === 'final'
            ? 'Closing final akan menyesuaikan saldo kas sistem menjadi kas fisik dan masuk rekonsiliasi dashboard.'
            : 'Cek sementara hanya mencatat posisi kas dan tidak mengubah saldo kas sistem.';
    }
};
...
function calculateClosingPhysical() {
    ...
    // Rumus: Selisih = Kas Fisik - Kas Sistem + Gantungan Piutang - Gantungan Utang
    const selisih = totalFisik - kasSistem + gantunganPiutang - gantunganUtang;
...
window.openClosingHistoryModal = function() {
    ...
    if(sorted.length === 0) {
        tbody.innerHTML = '<tr><td colspan="9" class="text-center">Belum ada riwayat closing.</td></tr>';
    }
};
...
window.saveEditClosing = function() {
    ...
    closings[idx].selisih = kasFisik - kasSistem + gPiutang - gUtang;
...
window.loadWaTemplatesSettings = function() {
    ...
    `).join('') || '<tr><td colspan="4" class="text-center text-muted">Belum ada template.</td></tr>';
};
...
window.saveWaTemplateFromSettings = function() {
    ...
    const next = { id, name, category, content, isDefault: isDefault || templates.length === 0 };
...
window.saveInitialBalances = function() {
    ...
    alert('Keseluruhan aset awal berhasil diterapkan.\n\nKurs pada tabel Pengaturan disimpan sebagai kurs modal awal terkunci dan tidak akan ikut berubah saat Manajemen Kurs disinkronkan ke Smartdeal.');
