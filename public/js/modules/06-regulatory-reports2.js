...
window.resetDatabase = async function() {
    ...
    let warn1 = confirm('PERINGATAN 1 DARI 3!\n\nReset ini akan mengosongkan Kas, Bank, Stok Valas, Transaksi, Mutasi, Closing, Nasabah, Pengeluaran, Penyesuaian, Kurs BI LKU, dan nomor invoice.\n\nTindakan ini SANGAT BERBAHAYA dan TIDAK BISA DIBATALKAN.');
    if (!warn1) { console.log("Reset dibatalkan di peringatan 1."); return; }

    let warn2 = confirm('PERINGATAN 2 DARI 3!\n\nData Master Valuta, Profil Perusahaan, Pengguna, dan setting referensi lain akan tetap dipertahankan.\nSeluruh histori transaksi dan data operasional akan HILANG SELAMANYA.\n\nAnda benar-benar yakin melanjutkan?');
    ...
    const resetArrayKeys = [
        ...
        'mc_hris_leave',
        'mc_old_money_stock',
        'mc_old_money_suppliers',
        'mc_old_money_trxs',
        'mc_bi_rates',
    ];

    const resetScalarValues = {
        mc_cash: 0,
        mc_bank_bca: 0,
        mc_bank_mandiri: 0,
        mc_old_money_cash: 0,
    ...
    // Reset balances
    Object.entries(resetScalarValues).forEach(([key, value]) => {
        localStorage.setItem(key, value.toString());
    });
    
    // Reset operational currency balances but keep master buy/sell settings.
    ...
    resetArrayKeys.forEach(key => {
        localStorage.setItem(key, JSON.stringify([]));
    });
    localStorage.setItem('mc_bi_rates_reset_at', new Date().toISOString());
    localStorage.removeItem('mc_invoice_seq');

    const syncTasks = [];
    if (typeof pushToUniversalDatastore === 'function') {
        Object.entries(resetScalarValues).forEach(([key, value]) => {
            syncTasks.push(pushToUniversalDatastore(key, value));
        });
        resetArrayKeys.forEach(key => {
            syncTasks.push(pushToUniversalDatastore(key, []));
        });
    }

    syncTasks.push(
        fetch('api/transactions/clear-all', { method: 'DELETE' }).then(response => {
            if (!response.ok) throw new Error('Gagal membersihkan transaksi MySQL');
            return response.json();
        })
    );

    syncTasks.push(
        fetch('api/customers/clear-all', { method: 'DELETE' }).then(response => {
            if (!response.ok) throw new Error('Gagal membersihkan nasabah MySQL');
            return response.json();
        })
    );
    ...
    try {
        ...
        console.error('Reset backend gagal:', error);
        alert('Reset lokal sudah dilakukan, tetapi sinkronisasi penghapusan ke server gagal. Jangan reload dulu, cek koneksi/Laragon lalu ulangi reset.');
        return;
    }
