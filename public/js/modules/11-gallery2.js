...
window.saveValasGalleryItem = async function() {
    ...
    const btn = document.querySelector('#valasGalleryModal .btn-primary');
    ...
    try {
        if (photo && photo.startsWith('data:') && typeof window.uploadBase64FileToFolder === 'function') {
            ...
            photo = await window.uploadBase64FileToFolder(photo, 'transactions/proofs', `valas-${code}`);
        }
    } catch (uploadError) {
    ...
    await saveValasGallery(galleryItems);
    btn.innerHTML = originalText;
    btn.disabled = false;

    alert('Data acuan valas berhasil disimpan!');
