...
function escapeGantunganHtml(value) {
...
window.initGantunganView = function() {
    ...
    renderGantunganPersonCards();
};

// Ringkas seluruh pinjaman/cicilan ke satu kartu per nama.
function renderGantunganPersonCards() {
    const host = document.getElementById('gantunganPersonCards');
    if (!host) return;
    const people = new Map();
    getGantungans().forEach(item => {
        const name = String(item.nama_nasabah || '').trim();
        if (!name || item.status === 'LUNAS') return;
        const key = name.toLocaleLowerCase('id-ID');
        if (!people.has(key)) people.set(key, { name, balance: 0, borrowed: 0, paid: 0, count: 0 });
        const person = people.get(key);
        const remaining = Math.max(0, Number(item.nominal || 0) - Number(item.terbayar || 0));
        const paid = Number(item.terbayar || 0);
        person.count += 1;
        if (item.tipe === 'PIUTANG') {
            person.balance += remaining;
            person.borrowed += Number(item.nominal || 0);
            person.paid += paid;
        } else {
            person.balance -= remaining;
        }
    });
    const cards = Array.from(people.values()).filter(person => person.balance !== 0).sort((a, b) => b.balance - a.balance);
    if (!cards.length) {
        host.innerHTML = '<div class="panel text-muted" style="grid-column:1/-1; padding:14px;">Tidak ada saldo gantungan aktif per nama.</div>';
        return;
    }
    host.innerHTML = cards.map(person => {
        const isReceivable = person.balance > 0;
        const encodedName = encodeURIComponent(person.name);
        return `<button type="button" class="panel" onclick="window.filterGantunganByName('${encodedName}')" style="text-align:left; cursor:pointer; border-top:4px solid ${isReceivable ? '#F59E0B' : '#3B82F6'}; padding:16px;">
            <div style="display:flex;justify-content:space-between;gap:10px;align-items:start;"><div><div style="font-size:.78rem;color:var(--text-muted);text-transform:uppercase;font-weight:700;">Kartu Gantungan</div><strong style="font-size:1.12rem;">${escapeGantunganHtml(person.name)}</strong></div><i class="fa-solid fa-user-tag" style="color:${isReceivable ? '#F59E0B' : '#3B82F6'};"></i></div>
            <div style="margin-top:13px;font-size:.78rem;color:var(--text-muted);">Total gantungan</div><div style="font-size:1.45rem;font-weight:800;color:${isReceivable ? '#F59E0B' : '#3B82F6'};">${formatIdr(Math.abs(person.balance))}</div>
            <div style="margin-top:8px;font-size:.78rem;color:var(--text-muted);">Pinjaman ${formatIdr(person.borrowed)} · Cicilan ${formatIdr(person.paid)} · ${person.count} catatan aktif</div>
        </button>`;
    }).join('');
}

window.filterGantunganByName = function(encodedName) {
    const input = document.getElementById('gantunganSearch');
    if (input) input.value = decodeURIComponent(encodedName || '');
    window.loadGantungansTable();
    document.getElementById('gantunganTable')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
};
...
window.loadGantungansTable = function() {
    ...
    filtered.forEach(item => {
        ...
        let typeBadge = '';
        if (item.tipe === 'PIUTANG') {
            typeBadge = '<span class="badge" style="background:#F59E0B; position:static;">Keluar / Pinjaman</span>';
        } else {
            typeBadge = '<span class="badge" style="background:#3B82F6; position:static;">Masuk / Kembalian</span>';
        }
