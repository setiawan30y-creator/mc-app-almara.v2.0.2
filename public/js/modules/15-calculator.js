...
(function() {
    ...
    window.toggleMultiCalculator = function() {
        ...
        if (calcEl.classList.contains('hidden')) {
            ...
            // Opening from the header always restores the full calculator.
            isMinimized = false;
            calcEl.classList.remove('is-minimized');
            calcEl.style.removeProperty('height');
            calcEl.style.removeProperty('max-height');
            document.getElementById('calc-expandable-content')?.style.removeProperty('display');
    ...
    window.toggleMinimizeCalculator = function() {
        ...
        if (isMinimized) {
            ...
            calcEl.classList.add('is-minimized');
        } else {
            // Restore the widget's original flex layout and rebuild the tabs.
            // Resetting inline limits prevents the sheet tabs from remaining
            // clipped after a previous minimize action.
