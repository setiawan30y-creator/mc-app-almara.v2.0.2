...
(function() {
    ...
    window.sendWaitingListContactWhatsApp = async function(id) {
        ...
        try {
            const fallbackMessage = `Halo ${item.name || 'Nasabah'},\n\nSalam dari KUPVA *Almara Putra Valasindo*. Ada yang dapat kami bantu?`;
            const message = window.buildWaMessageForPurpose('waiting', fallbackMessage, { customerName: item.name, invoice: item.id });
            await window.sendWhatsAppGateway(waPhone, message, { reference: item.id });
