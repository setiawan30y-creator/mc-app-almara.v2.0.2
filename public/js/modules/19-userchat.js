...
(function() {
    ...
    const chatEmojiList = [
        '\u{1F600}', '\u{1F601}', '\u{1F602}', '\u{1F60A}', '\u{1F60D}', '\u{1F60E}',
        '\u{1F622}', '\u{1F62D}', '\u{1F621}', '\u{1F44D}', '\u{1F64F}', '\u{1F44F}',
        '\u{1F4AA}', '\u{2705}', '\u{274C}', '\u{26A0}\u{FE0F}', '\u{1F4B0}', '\u{1F4B5}',
        '\u{1F4CC}', '\u{1F4CE}', '\u{1F4DD}', '\u{1F525}', '\u{2B50}', '\u{2764}\u{FE0F}'
    ];
    ...
    function renderEmojiPicker() {
        const picker = document.getElementById('userChatEmojiPicker');
        if (!picker) return;
        picker.innerHTML = chatEmojiList.map(emoji => `
            <button type="button" onclick="window.insertUserChatEmoji('${emoji}')" style="width:30px;height:30px;border:none;border-radius:6px;background:rgba(255,255,255,.04);cursor:pointer;font-size:1.05rem;">${emoji}</button>
        `).join('');
    }

    window.toggleUserChatEmojiPicker = function() {
        const picker = document.getElementById('userChatEmojiPicker');
        if (picker) picker.classList.toggle('hidden');
    };

    window.insertUserChatEmoji = function(emoji) {
    ...
    window.openUserChatFilePicker = function() {
        document.getElementById('userChatFileInput')?.click();
    };

    window.openUserChatImagePicker = function() {
        document.getElementById('userChatImageInput')?.click();
    };

    window.handleUserChatFileSelected = function(input) {
    ...
    function renderAttachmentPreview() {
        const wrap = document.getElementById('userChatAttachmentPreview');
        const thumb = document.getElementById('userChatAttachmentThumb');
        const name = document.getElementById('userChatAttachmentName');
        const size = document.getElementById('userChatAttachmentSize');
        if (!wrap || !thumb || !name || !size || !selectedChatAttachment) return;

        const isImage = selectedChatAttachment.mime.startsWith('image/');
        thumb.innerHTML = isImage
            ? `<img src="${selectedChatAttachment.data}" style="width:100%;height:100%;object-fit:cover;">`
            : `<i class="fa-solid fa-file"></i>`;
        name.textContent = selectedChatAttachment.name;
        size.textContent = `${selectedChatAttachment.mime || 'file'} • ${formatFileSize(selectedChatAttachment.size)}`;
        wrap.classList.remove('hidden');
    }

    window.clearUserChatAttachment = function() {
    ...
    function renderMessagesList(messages, append = false) {
        ...
        const html = messages.map(m => {
            ...
            const attachmentName = m.attachment_name || '';
            ...
            const attachmentHtml = hasAttachment
                ? (String(attachmentMime).startsWith('image/')
                    ? `
                        <a href="${attachmentPath}" target="_blank" style="display:block; margin-bottom:7px; border-radius:8px; overflow:hidden; border:1px solid rgba(255,255,255,.18); background:rgba(0,0,0,.18);">
                            <img src="${attachmentPath}" alt="${escapeHtml(attachmentName)}" style="display:block; max-width:240px; max-height:180px; object-fit:cover;">
                        </a>
                    `
                    : `
                        <a href="${attachmentPath}" target="_blank" download style="display:flex; align-items:center; gap:8px; color:${color}; text-decoration:none; margin-bottom:7px; padding:8px; border-radius:8px; background:rgba(255,255,255,.08); border:1px solid rgba(255,255,255,.12);">
                            <i class="fa-solid fa-file-arrow-down"></i>
                            <span style="font-weight:700; max-width:220px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${escapeHtml(attachmentName || 'Lampiran')}</span>
                            <small style="opacity:.75;">${formatFileSize(m.attachment_size)}</small>
                        </a>
                    `)
                : '';
        ...
        try {
            ...
            const response = await fetch('/api/user-chats/messages', {
                ...
                body: JSON.stringify({
                    ...
                    attachment_data: selectedChatAttachment?.data || null,
                    attachment_name: selectedChatAttachment?.name || null,
                    attachment_mime: selectedChatAttachment?.mime || null,
                    attachment_size: selectedChatAttachment?.size || null
                })
            });
            ...
            if (response.ok) {
                if (result.status === 'success') {
                    input.value = '';
                    window.clearUserChatAttachment();
