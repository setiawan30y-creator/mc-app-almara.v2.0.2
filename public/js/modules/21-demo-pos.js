...
(function () {
    ...
    window.demoPosProcess = function () { if (!cart.length) return alert('Keranjang demo masih kosong.'); const list = stock(); const history = safeGet(TRX_KEY); const oldIndex = editingTransactionId ? history.findIndex(item => item.id === editingTransactionId) : -1; const oldTransaction = oldIndex >= 0 ? history[oldIndex] : null;
        // Kembalikan dampak stok transaksi lama dahulu agar edit tidak menggandakan stok.
        if (oldTransaction) oldTransaction.items.forEach(line => { const item = list.find(row => row.id === line.id); if (item) item.stock = Number(item.stock || 0) + (line.type === 'BELI' ? -line.amount : line.amount); });
        for (const line of cart) { const item = list.find(row => row.id === line.id); if (!item || (line.type === 'JUAL' && Number(item.stock || 0) < line.amount)) return alert('Stok demo ' + line.code + ' tidak mencukupi.'); }
        ...
        const trx = { id: oldTransaction?.id || ('demo_trx_' + Date.now()), invoice: document.getElementById('demoPosInvoice').value || newInvoice(), date: document.getElementById('demoPosDate').value, customer: document.getElementById('demoPosCustomer').value.trim(), payment: document.getElementById('demoPosPayment').value, note: document.getElementById('demoPosNote').value.trim(), items: cart.map(item => ({ ...item })), total: cart.reduce((sum, item) => sum + item.total, 0), createdAt: oldTransaction?.createdAt || new Date().toISOString(), updatedAt: new Date().toISOString() };
        if (oldIndex >= 0) history[oldIndex] = trx; else history.push(trx); save(STOCK_KEY, list); save(TRX_KEY, history); lastTransaction = trx; renderHistory(); document.getElementById('demoPosPrint').classList.remove('hidden'); alert(oldTransaction ? 'Transaksi demo berhasil diperbarui.' : 'Transaksi demo berhasil disimpan. Data POS utama tidak berubah.'); resetForm(); };
