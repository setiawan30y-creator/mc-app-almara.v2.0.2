...
if (!window.__almaraRealtimeStoragePatch) {
    ...
    const localOnlyKeys = new Set([
        ...
        // Tab/form POS adalah preferensi lokal; tidak boleh dipulihkan oleh
        // snapshot datastore dari perangkat atau sesi lama.
        'mc_pos_draft_sessions_v1',
        'mc_pos_active_draft_id_v1'
    ]);
        ...
        if (response.ok) {
            ...
            if (result.status === 'success' && result.data) {
                ...
                for (const [key, rawValue] of Object.entries(result.data)) {
                    ...
                    if (key === 'mc_pos_draft_sessions_v1' || key === 'mc_pos_active_draft_id_v1') continue;
                    if (key === 'mc_customers') {
...
async function hasCustomerResetLock() {
    const localResetAt = localStorage.getItem('mc_customers_reset_at');
    if (localResetAt) return true;
...
async function syncFromMySQL_Customers(options = {}) {
    const { refreshUi = false, silent = false } = options;
    const activePushes = window.__almaraActiveCustomerPushes || 0;
    const lastWriteAt = window.__almaraLastLocalCustomerWriteAt || 0;
    if (activePushes > 0 || (Date.now() - lastWriteAt) < 5000) {
        if (!silent) console.log('[Sync-Skip] Penarikan nasabah ditunda karena penyimpanan masih aktif/baru.');
        return;
    }
    try {
        ...
        if (response.ok) {
            const data = await response.json();
            // Respons GET dapat dimulai sebelum import, tetapi baru selesai sesudah
            // import selesai. Jangan biarkan snapshot lama menimpa hasil baru.
            if ((window.__almaraActiveCustomerPushes || 0) > 0 || (Date.now() - (window.__almaraLastLocalCustomerWriteAt || 0)) < 5000) {
                if (!silent) console.log('[Sync-Skip] Respons nasabah lama diabaikan setelah penyimpanan baru.');
                return;
            }
...
async function saveToMySQL_Customer(customerObject) {
    window.__almaraActiveCustomerPushes = (window.__almaraActiveCustomerPushes || 0) + 1;
    try {
        const request = window.authFetch || window.fetch;
        ...
        window.__almaraActiveCustomerPushes = Math.max(0, (window.__almaraActiveCustomerPushes || 0) - 1);
        window.__almaraLastLocalCustomerWriteAt = Date.now();
    }
}

async function saveBulkToMySQL_Customers(customersArray, options = {}) {
    window.__almaraActiveCustomerPushes = (window.__almaraActiveCustomerPushes || 0) + 1;
    try {
        const request = window.authFetch || window.fetch;
        const list = Array.isArray(customersArray) ? customersArray : [];
        // Tetap satu klik untuk ribuan data, tetapi request dipecah agar tidak
        // melewati batas Nginx. Empat batch diproses paralel agar tetap cepat.
        const batchSize = 100;
        const concurrency = 4;
        const batches = [];
        for (let offset = 0; offset < list.length; offset += batchSize) {
            batches.push(list.slice(offset, offset + batchSize));
        }
        let successCount = 0;
        let failedCount = 0;
        const errors = [];
        ...
        const sendBatch = async (batch, batchIndex) => {
            ...
            let response = await request('api/customers/bulk', requestOptions);
            if (response.status === 419 && await refreshCsrfTokenFromServer()) {
                response = await request('api/customers/bulk', requestOptions);
            }
            ...
            try {
                ...
                const title = rawResponse.match(/<title>([^<]+)<\/title>/i)?.[1];
                result = { status: 'error', message: title || `Respons server tidak valid (${response.status})` };
            }
            if (!response.ok || result.status === 'error') {
                failedCount += batch.length;
                errors.push(`Batch ${batchIndex + 1}: ${result.message || `HTTP ${response.status}`}`);
            } else {
                successCount += Number(result.success_count ?? batch.length) || 0;
                failedCount += Number(result.failed_count ?? 0) || 0;
                if (Array.isArray(result.errors)) errors.push(...result.errors);
            }
            completedBatches++;
            if (typeof options.onProgress === 'function') {
                options.onProgress({ completedBatches, totalBatches: batches.length, successCount, failedCount });
            }
        };
        const worker = async () => {
            while (nextBatchIndex < batches.length) {
                const batchIndex = nextBatchIndex++;
                await sendBatch(batches[batchIndex], batchIndex);
            }
        };
        await Promise.all(Array.from({ length: Math.min(concurrency, batches.length) }, worker));
        const ok = failedCount === 0;
        const message = ok
            ? `Berhasil menyimpan ${successCount} data nasabah dalam ${batches.length} batch.`
            : `Sebagian data gagal disimpan. Sukses: ${successCount}, gagal: ${failedCount}.`;
        console.log('Sinkronisasi Bulk Nasabah MySQL:', message);
        return {
            ok,
            status: lastStatus,
            message,
...
function notifyNewRemoteTransactionsIfNeeded(beforeTransactions, nextTransactions, options = {}) {
    ...
    // Penyegaran form non-transaksi (misalnya simpan nasabah) tidak boleh
    // memunculkan popup transaksi lama yang baru terbaca saat sinkronisasi.
    if (!force && Date.now() < (window.__almaraSuppressTransactionNotificationsUntil || 0)) {
        markTransactionKeysSeenForNotification(nextTransactions);
        return;
    }
...
window.suppressTransactionNotifications = function(milliseconds = 8000) {
    window.__almaraSuppressTransactionNotificationsUntil = Date.now() + Math.max(0, Number(milliseconds) || 0);
    try {
        const current = JSON.parse(localStorage.getItem('mc_transactions') || '[]');
        markTransactionKeysSeenForNotification(current);
    } catch (error) {
        console.warn('Gagal menandai notifikasi transaksi saat simpan nasabah:', error);
    }
};
