            ...
            <div class="view-container">
                    ...
                    <div class="panel header-panel flex-between" style="border-left: 4px solid #ef4444;">
                        <div>
                            <div style="display:flex; align-items:center; gap:12px;">
                                <h2 class="text-red" style="margin:0;"><i class="fa-solid fa-shield-halved"></i> RWT</h2>
                                <span class="badge" style="background:#10b981; color:white; padding:5px 12px; border-radius:20px; font-size:0.78rem; font-weight:600;"><i class="fa-solid fa-lock"></i> Audit Protected (Tidak Dapat Dihapus)</span>
                            </div>
                                ...
                                reader.onload = async function(e) {
                                    try {
                                        ...
                                        jsonData.forEach((row, i) => {
                                            ...
                                            const tipeValue = getHeaderCell(row, ['Tipe', 'KN(TIPE)', 'KN Tipe', 'KN', 'customer_type'], 1);
                                            const namaValue = getHeaderCell(row, ['Nama', 'Nama Lengkap', 'NAMA LENGKAP', 'name'], 2);
                                            ...
                                            const idpjk = getHeaderCell(row, ['IDPJK'], undefined) !== undefined ? String(getHeaderCell(row, ['IDPJK'], undefined)).trim() : '-';
                                            const warga_negara = getHeaderCell(row, ['Warga_Negara', 'Warga Negara'], 3) !== undefined ? String(getHeaderCell(row, ['Warga_Negara', 'Warga Negara'], 3)).trim() : '-';
                                            const jenis_kelamin = getHeaderCell(row, ['Jenis_Kelamin', 'Jenis Kelamin'], 4) !== undefined ? String(getHeaderCell(row, ['Jenis_Kelamin', 'Jenis Kelamin'], 4)).trim() : '-';
                                            const tempat_lahir = getHeaderCell(row, ['Tempat_Lahir', 'Tempat Lahir'], 5) !== undefined ? String(getHeaderCell(row, ['Tempat_Lahir', 'Tempat Lahir'], 5)).trim() : '-';
                                            const tanggal_lahir = getHeaderCell(row, ['Tanggal_Lahir', 'Tanggal Lahir'], 6) !== undefined ? String(getHeaderCell(row, ['Tanggal_Lahir', 'Tanggal Lahir'], 6)).trim() : '-';
                                            const alamat = getHeaderCell(row, ['Alamat'], 7) !== undefined ? String(getHeaderCell(row, ['Alamat'], 7)).trim() : '-';
                                            const pekerjaan = getHeaderCell(row, ['Pekerjaan'], 8) !== undefined ? String(getHeaderCell(row, ['Pekerjaan'], 8)).trim() : '-';
                                            const no_hp = getHeaderCell(row, ['No_HP', 'No HP', 'NO TLP', 'No TLP', 'No Telp', 'Phone'], 9) !== undefined ? String(getHeaderCell(row, ['No_HP', 'No HP', 'NO TLP', 'No TLP', 'No Telp', 'Phone'], 9)).trim() : '-';
                                            const no_rekening = getHeaderCell(row, ['No_Rekening', 'No Rekening'], 10) !== undefined ? String(getHeaderCell(row, ['No_Rekening', 'No Rekening'], 10)).trim() : '';
                                            const no_ktp = getHeaderCell(row, ['No_KTP', 'No KTP', 'NO KTP', 'identity_number'], 11) !== undefined ? String(getHeaderCell(row, ['No_KTP', 'No KTP', 'NO KTP', 'identity_number'], 11)).trim() : '-';
                                            const selain_ktp = getHeaderCell(row, ['Selain_KTP', 'Selain KTP', 'ID Lain'], 12) !== undefined ? String(getHeaderCell(row, ['Selain_KTP', 'Selain KTP', 'ID Lain'], 12)).trim() : '-';
                                            const no_cif = getHeaderCell(row, ['No_CIF', 'No CIF', 'id_cif'], 13) !== undefined ? String(getHeaderCell(row, ['No_CIF', 'No CIF', 'id_cif'], 13)).trim() : '';
                                            const npwp = getHeaderCell(row, ['NPWP'], 14) !== undefined ? String(getHeaderCell(row, ['NPWP'], 14)).trim() : '';
                                            const local_id = getHeaderCell(row, ['Local_ID', 'Local ID'], 15) !== undefined ? String(getHeaderCell(row, ['Local_ID', 'Local ID'], 15)).trim() : '';
                                            ...
                                            const tglDaftarValue = getHeaderCell(row, ['Tgl_Daftar', 'Tgl Daftar', 'Tanggal Daftar'], 16);
                ...
                <section id="settings-view" class="view-section hidden">
                    ...
                    <div class="panel mt-4 settings-tab-content hidden" id="settings-tab-danger" style="display:none; border: 1px solid #ef4444;">
                        <h2 style="color: #ef4444;"><i class="fa-solid fa-triangle-exclamation"></i> Manajemen Data & Reset Per Modul (Danger Zone)</h2>
                        <p class="mt-2 text-muted">Simpan rutinitas database lokal Anda sebelum melakukan penghapusan atau reset total.</p>
                        <div class="mt-3" style="padding: 12px 14px; background: rgba(239, 68, 68, 0.08); border-left: 3px solid #ef4444; border-radius: 6px;">
                            <div style="font-weight: 700; color: #ef4444; margin-bottom: 12px; font-size: 1rem;"><i class="fa-solid fa-list-check"></i> Pilih Halaman / Modul Yang Ingin Di-Reset:</div>
                            <div id="dangerZoneModuleSelectorContainer">
                                <div style="display: flex; justify-content: flex-end; gap: 8px; margin-bottom: 12px;">
                                    <button type="button" class="btn btn-sm btn-outline" onclick="window.toggleSelectAllResetModules(true)">Pilih Semua</button>
                                    <button type="button" class="btn btn-sm btn-outline" onclick="window.toggleSelectAllResetModules(false)">Kosongkan Pilihan</button>
                                </div>

                                <div class="row" style="row-gap: 12px;">
                                    <div class="col-md-6 col-lg-4">
                                        <label class="d-flex align-items-center gap-2" style="cursor: pointer; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 6px; border: 1px solid rgba(255,255,255,0.1);">
                                            <input type="checkbox" class="reset-module-checkbox" value="transactions" style="width: 18px; height: 18px; accent-color: #ef4444;">
                                            <div>
                                                <strong>Data Transaksi Operasional</strong>
                                                <div class="text-muted" style="font-size: 0.78rem;">Riwayat jual/beli & mutasi kasir</div>
                                            </div>
                                        </label>
                                    </div>
                                    <div class="col-md-6 col-lg-4">
                                        <label class="d-flex align-items-center gap-2" style="cursor: pointer; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 6px; border: 1px solid rgba(255,255,255,0.1);">
                                            <input type="checkbox" class="reset-module-checkbox" value="customers" style="width: 18px; height: 18px; accent-color: #ef4444;">
                                            <div>
                                                <strong>Data Nasabah (KYC)</strong>
                                                <div class="text-muted" style="font-size: 0.78rem;">Daftar CIF & foto identitas nasabah</div>
                                            </div>
                                        </label>
                                    </div>
                                    <div class="col-md-6 col-lg-4">
                                        <label class="d-flex align-items-center gap-2" style="cursor: pointer; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 6px; border: 1px solid rgba(255,255,255,0.1);">
                                            <input type="checkbox" class="reset-module-checkbox" value="cash_bank" style="width: 18px; height: 18px; accent-color: #ef4444;">
                                            <div>
                                                <strong>Saldo Kas & Bank</strong>
                                                <div class="text-muted" style="font-size: 0.78rem;">Kas Tunai, Bank BCA, & Mandiri</div>
                                            </div>
                                        </label>
                                    </div>
                                    <div class="col-md-6 col-lg-4">
                                        <label class="d-flex align-items-center gap-2" style="cursor: pointer; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 6px; border: 1px solid rgba(255,255,255,0.1);">
                                            <input type="checkbox" class="reset-module-checkbox" value="currency_stock" style="width: 18px; height: 18px; accent-color: #ef4444;">
                                            <div>
                                                <strong>Stok Valas / FISIK</strong>
                                                <div class="text-muted" style="font-size: 0.78rem;">Kosongkan jumlah stok lembaran valas</div>
                                            </div>
                                        </label>
                                    </div>
                                    <div class="col-md-6 col-lg-4">
                                        <label class="d-flex align-items-center gap-2" style="cursor: pointer; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 6px; border: 1px solid rgba(255,255,255,0.1);">
                                            <input type="checkbox" class="reset-module-checkbox" value="mutations" style="width: 18px; height: 18px; accent-color: #ef4444;">
                                            <div>
                                                <strong>Mutasi & Penyesuaian Saldo</strong>
                                                <div class="text-muted" style="font-size: 0.78rem;">Jejak penyesuaian & transfer antar kas</div>
                                            </div>
                                        </label>
                                    </div>
                                    <div class="col-md-6 col-lg-4">
                                        <label class="d-flex align-items-center gap-2" style="cursor: pointer; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 6px; border: 1px solid rgba(255,255,255,0.1);">
                                            <input type="checkbox" class="reset-module-checkbox" value="closings" style="width: 18px; height: 18px; accent-color: #ef4444;">
                                            <div>
                                                <strong>Riwayat Closing & Laporan LKU</strong>
                                                <div class="text-muted" style="font-size: 0.78rem;">Jurnal penutupan harian & LKU</div>
                                            </div>
                                        </label>
                                    </div>
                                    <div class="col-md-6 col-lg-4">
                                        <label class="d-flex align-items-center gap-2" style="cursor: pointer; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 6px; border: 1px solid rgba(255,255,255,0.1);">
                                            <input type="checkbox" class="reset-module-checkbox" value="expenses_assets" style="width: 18px; height: 18px; accent-color: #ef4444;">
                                            <div>
                                                <strong>Pengeluaran & Penyusutan Aset</strong>
                                                <div class="text-muted" style="font-size: 0.78rem;">Beban biaya & daftar aset kantor</div>
                                            </div>
                                        </label>
                                    </div>
                                    <div class="col-md-6 col-lg-4">
                                        <label class="d-flex align-items-center gap-2" style="cursor: pointer; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 6px; border: 1px solid rgba(255,255,255,0.1);">
                                            <input type="checkbox" class="reset-module-checkbox" value="old_money_trxs" style="width: 18px; height: 18px; accent-color: #ef4444;">
                                            <div>
                                                <strong>Uang Lama/Koin (Riwayat)</strong>
                                                <div class="text-muted" style="font-size: 0.78rem;">Riwayat transaksi koin/uang kuno</div>
                                            </div>
                                        </label>
                                    </div>
                                    <div class="col-md-6 col-lg-4">
                                        <label class="d-flex align-items-center gap-2" style="cursor: pointer; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 6px; border: 1px solid rgba(255,255,255,0.1);">
                                            <input type="checkbox" class="reset-module-checkbox" value="old_money_suppliers" style="width: 18px; height: 18px; accent-color: #ef4444;">
                                            <div>
                                                <strong>Uang Lama/Koin (Supplier)</strong>
                                                <div class="text-muted" style="font-size: 0.78rem;">Daftar supplier koin/uang kuno</div>
                                            </div>
                                        </label>
                                    </div>
                                    <div class="col-md-6 col-lg-4">
                                        <label class="d-flex align-items-center gap-2" style="cursor: pointer; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 6px; border: 1px solid rgba(255,255,255,0.1);">
                                            <input type="checkbox" class="reset-module-checkbox" value="old_money_items" style="width: 18px; height: 18px; accent-color: #ef4444;">
                                            <div>
                                                <strong>Uang Lama/Koin (Definisi/Katalog)</strong>
                                                <div class="text-muted" style="font-size: 0.78rem;">Katalog & definisi barang koin baru</div>
                                            </div>
                                        </label>
                                    </div>
                                    <div class="col-md-6 col-lg-4">
                                        <label class="d-flex align-items-center gap-2" style="cursor: pointer; padding: 10px; background: rgba(239,68,68,0.1); border-radius: 6px; border: 1px solid rgba(239,68,68,0.3);">
                                            <input type="checkbox" class="reset-module-checkbox" value="rwt" style="width: 18px; height: 18px; accent-color: #ef4444;">
                                            <div>
                                                <strong class="text-danger"><i class="fa-solid fa-shield-halved"></i> Data RWT (Audit Transaksi)</strong>
                                                <div class="text-muted" style="font-size: 0.78rem;">Tabel audit permanen & independen</div>
                                            </div>
                                        </label>
                                    </div>
                                    <div class="col-md-6 col-lg-4">
                                        <label class="d-flex align-items-center gap-2" style="cursor: pointer; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 6px; border: 1px solid rgba(255,255,255,0.1);">
                                            <input type="checkbox" class="reset-module-checkbox" value="hris" style="width: 18px; height: 18px; accent-color: #ef4444;">
                                            <div>
                                                <strong>HRIS & Absensi Karyawan</strong>
                                                <div class="text-muted" style="font-size: 0.78rem;">PIN absensi & data pegawai HRIS</div>
                                            </div>
                                        </label>
                                    </div>
                                </div>

                                <div style="margin-top: 14px; padding: 10px 12px; background: rgba(16, 185, 129, 0.1); border-left: 3px solid #10b981; border-radius: 4px; font-size: 0.84rem; color: #a7f3d0;">
                                    <i class="fa-solid fa-shield-halved"></i> <strong>Sistem Keamanan Reset:</strong> Master Valuta, Profil Perusahaan, dan Akun Login Pengguna <u>TIDAK DAPAT DIHAPUS</u>. Data RWT hanya terhapus jika Anda secara spesifik mencentang opsi RWT di atas.
                                </div>
                            ...
                            <div class="d-flex gap-2 mt-4 flex-wrap">
                                ...
                                <button type="button" class="btn btn-danger" onclick="executeSelectiveReset()"><i class="fa-solid fa-trash-can"></i> Reset Modul Terpilih</button>
                            </div>
                        </div>
