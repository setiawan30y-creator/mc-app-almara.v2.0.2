...
<title>MC-Almara | Theme Settings</title>
<link rel="stylesheet" href="{{ asset('css/mc-theme-engine.css?v=20260924-1') }}">
<style>
...
<div class="field"><label>{{ $f[1] }}</label><div class="color"><input type="color" id="theme-{{ $f[0] }}" data-theme-field><input type="text" id="theme-{{ $f[0] }}-text" value="" oninput="document.getElementById('theme-{{ $f[0] }}').value=this.value;document.getElementById('theme-{{ $f[0] }}').dispatchEvent(new Event('input'))"></div></div>
@endforeach
</div></div>
...
<p class="hint">Perubahan di preview bersifat langsung. Klik <b>Simpan Tema</b> untuk menyimpan sebagai tema global pada browser ini.</p>
</div>
<div class="cardbox preview"><h2 style="margin-top:0">Live Preview</h2><div id="theme-preview" class="mock"><div class="mock-side"><strong>MC-ALMARA</strong><div class="active">Dashboard</div><div>Transaksi</div><div>Nasabah</div><div>Stock Valas</div><div>Closing</div></div><div class="mock-main"><div class="mock-head"><b>Dashboard</b><span style="color:var(--p-muted)">Teller</span></div><div class="mock-cards"><div class="mock-card"><small>Total Kas</small><h2>Rp 125.000.000</h2></div><div class="mock-card"><small>Mutasi Bank</small><h2>Rp 45.000.000</h2></div></div><div class="mock-card" style="margin-top:12px"><small>Contoh transaksi</small><p>USD 100 × Rp 16.500</p><span class="mock-button">Simpan Transaksi</span></div></div></div></div>
</div></div>
<div id="theme-toast" class="toast"></div>
<script src="{{ asset('js/mc-theme-engine.js?v=20260924-1') }}"></script><script src="{{ asset('js/mc-theme-settings.js?v=20260924-1') }}"></script>
</body></html>
